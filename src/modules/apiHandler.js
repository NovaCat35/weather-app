...
async function fetchInfo(location, zip = null){
   ...
   try {
      if(zip) {
          currResponsePromise = fetch(`http://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${location}&q=${zip}`)
          forecastResponsePromise = fetch(`http://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${location}&q=${zip}`)
      } else {
